...
Web scraping steps:
  ...
  5) **Testing and Appending**
     The final step checks whether the same amount of pitches were collected across the three methods of data collection, if false an error is raised. If true,  then the data from each of the methods are converted to a pyspark dataframe and are append in the repspective tables in the bronze schema.
